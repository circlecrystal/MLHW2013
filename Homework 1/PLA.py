...
def nextxy_readlist(readlist, loop, n, listxy):
    return readxy(readlist[loop % n], listxy)
...
def pla_core(path):
    ...
    while True:
        (x, y) = nextxy(loop % n, listxy)
        t = update_or_pass(w, x, y)
        loop += 1
        if t == -1:
            s += 1
            if s == n:
...
def pla_readlist_core(path, readlist, longn=1):
    n = getn(path)
    d = getd(path)
    listxy = xylist(path)
    w = [0] * d # w0
    ...
    loop = 0 # number of loops
    while True:
        (x, y) = nextxy_readlist(readlist, loop, n, listxy)
...
def pla_fixed_random(path, times=2000, longn=0.5):
    ...
    for i in range(times):
        seed()
        shuffle(readlist)
        (w, count) = pla_readlist_core(path, readlist, longn)
        sumofcounts += count
